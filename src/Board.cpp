...
bool Board::makeMove(Player *const currentPlayer, int const &sourceX, int const &sourceY,
    ...
    movesPlayed.emplace_back(currentPlayer->isWhiteSide(), sourceX, sourceY, destinationX, destinationY, sourcePiece,
                             killedPiece);
...
void Board::executePromotion(Player *const currentPlayer, int const &destinationX, int const &destinationY) {
    ...
    switch (promotionPiece) {
        ...
        default:
            throw std::invalid_argument("Wrong promotion input!");
    }
}

bool
Board::isValidCastling(int const &sourceX, int const &sourceY, int const &destinationX, int const &destinationY) const {
...
void
Board::executeCastlingMove(int const &sourceX, int const &sourceY, int const &destinationX, int const &destinationY) {
...
bool Board::isEnPassant(int const &sourceX, int const &sourceY,
    ...
    std::vector<int> target = EnPassantTarget();
    if (!(getSquare(sourceX, sourceY).getPiece()->getPieceType() == PieceType::PAWN &&
          destinationX == target[0] && destinationY == target[1]))
        return false;
...
std::vector<int> Board::EnPassantTarget() const {
    if (movesPlayed.empty()) {
    ...
    for (int i = 7; i >= 0; i--) {
        ...
        for (int j = 0; j < 8; j++) {
            ...
            switch (p->getPieceType()) {
                case PieceType::KING:
                    p->isWhite() ? std::cout << " K " : std::cout << " k ";
                    break;
                case PieceType::QUEEN:
                    p->isWhite() ? std::cout << " Q " : std::cout << " q ";
                    break;
                case PieceType::BISHOP:
                    p->isWhite() ? std::cout << " B " : std::cout << " b ";
                    break;
                case PieceType::KNIGHT:
                    p->isWhite() ? std::cout << " N " : std::cout << " n ";
                    break;
                case PieceType::ROOK:
                    p->isWhite() ? std::cout << " R " : std::cout << " r ";
                    break;
                case PieceType::PAWN:
                    p->isWhite() ? std::cout << " P " : std::cout << " p ";
                    break;
            }
        }
        std::cout << std::endl;
