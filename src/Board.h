...
class Board {
 public:
  ...
  bool makeMove(Player currentPlayer, int const& sourceX, int const& sourceY, int const& killedX, int const& killedY);
  ...
  void executePromotion(Player currentPlayer, int const& destinationX, int const& destinationY);
  bool isValidCastling(int const& sourceX, int const& sourceY, int const& destinationX, int const& destinationY) const;
  ...
  std::tuple<int, int> EnPassantTarget() const;
